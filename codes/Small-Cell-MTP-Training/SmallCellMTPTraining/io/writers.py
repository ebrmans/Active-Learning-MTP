...
def writeVASPInput_fcc(fileName: str, taskProperties: dict, vasp_input_folder: str) -> str:
    """
    
    vasp_input_folder : folder which contains required INCAR, POSCAR and POTCAR files 

    YM: This is meant to be flexible, use of '{' and '}' characters to replace keywords 
    
    Creates a VASP input, needing
    properties = [
        "atomPositions",
        "atomTypes",
        "superCell",
...
def writeMDInput_fcc(fileName: str, jobProperties: dict):
        ...
        for j in range(ny):
            for k in range(nz):
                for base_atom in [
                    ...
                    (0.5, 0, 0.5),
                    (0.5, 0.5, 0)
                ]:  # YM note: change to FCC basis atoms
                    if (
                        n_created < num_atoms
                    ):  # This check is important! Before, we created too many atoms
                        x = (
    ...
    newMDInput = templates.mdInputTemplate_fcc.format(
        base=jobProperties["latticeParameter"],
        ttt=jobProperties["temperature"],
        ppp=jobProperties["pressure"],
        pot=jobProperties["potFile"],
        ...
        ydim=jobProperties["boxDimensions"][1],
        zdim=jobProperties["boxDimensions"][2],
        threshold_save=jobProperties["threshold_save"],
        threshold_break=jobProperties["threshold_break"],
        num_elem=num_elem,
        mass_block=mass_block,
        create_atoms_block=create_atoms_block,
    )
